...
class Tetrimino(ABC):
    ...
    def get_visible_rows_in_collision_columns(self, grid: Grid) -> List[VisibleRow]:
        ...
        """
        return [
            grid.get_visible_row_by_column(column + self.offset)
            for column in self.collision_columns
        ]
    ...
    @abstractmethod
    def get_rows_for_placement(
        self, grid: Grid, visible_rows: List[VisibleRow]
    ) -> List[Row]:
        pass
...
class TBlock(Tetrimino):
    ...
    def get_rows_for_placement(
        ...
        if visible_rows[0] > visible_rows[1] and visible_rows[0] > visible_rows[2]:
            rows_for_placement = grid.get_next_n_rows(
                visible_rows[0], len(self.shape), True
            )
        # Case 2: T block hangs on right side
        elif visible_rows[2] > visible_rows[1] and visible_rows[2] > visible_rows[0]:
            rows_for_placement = grid.get_next_n_rows(
                visible_rows[2], len(self.shape), True
            )
...
class SBlock(Tetrimino):
    ...
    def get_rows_for_placement(
        ...
        # Case 2: the bottom row of the S block is immediately on top of another block
            ...
            rows_for_placement = grid.get_next_n_rows(
                highest_visible_row, len(self.shape)
            )
...
class ZBlock(Tetrimino):
    ...
    def get_rows_for_placement(
        ...
        # Case 2: the bottom row of the S block is immediately on top of another block
            ...
            highest_visible_row = (
                visible_rows[2]
                if visible_rows[2] > visible_rows[1]
                else visible_rows[1]
            )
            rows_for_placement = grid.get_next_n_rows(
                highest_visible_row, len(self.shape)
